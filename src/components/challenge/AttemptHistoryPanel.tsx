...
import { ChevronDown, Clock3, Code2, FileJson, FileSpreadsheet, History, PieChart, Zap } from 'lucide-react';
import type { Attempt } from '../../types';
import { exportService } from '../../services/exportService';

interface AttemptHistoryPanelProps { attempts: Attempt[]; challengeId?: string; userId?: string; }

export function AttemptHistoryPanel({ attempts, challengeId, userId }: AttemptHistoryPanelProps) {
  const [showSummary, setShowSummary] = React.useState(false);
  if (!attempts.length) return <div className="attempt-history-empty"><span><History size={31} /></span><h2 id="history-title">Sem histórico ainda</h2><p>Suas tentativas aparecerão aqui para que você possa acompanhar sua evolução.</p></div>;
...
function AttemptHistoryItem({ attempt, number }: { attempt: Attempt; number: number }) {
  ...
  return <article className={`attempt-item ${statusClass}`}>
    <button type="button" onClick={() => setIsExpanded(value => !value)} aria-expanded={isExpanded} aria-controls={contentId}>
      <span className="attempt-item__marker" aria-hidden="true" /><span className="attempt-item__copy"><strong>Tentativa {number}: {attempt.category || 'Registrada'}</strong><small>{formattedDate && <span><Clock3 size={13} />{formattedDate}</span>}<span><Zap size={13} />{attempt.tipsUsed.length} dicas</span>{attempt.isLocal && <em>Offline</em>}</small></span><ChevronDown size={19} />
    </button>
    <AnimatePresence>{isExpanded && <motion.div id={contentId} className="attempt-item__content" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}>
      <section><h3>Análise da tentativa</h3><p>{attempt.analysisSummary || 'Análise pedagógica registrada para esta tentativa.'}</p></section>
      <section className="attempt-tip"><h3><Zap size={15} />Hipótese de dificuldade</h3><p>{attempt.difficultyHypothesis || 'Reflexão sobre o raciocínio lógico.'}</p></section>
      <section><h3><Code2 size={15} />Código enviado</h3><pre className="custom-scrollbar"><code>{attempt.code}</code></pre></section>
    </motion.div>}</AnimatePresence>
  </article>;
}
